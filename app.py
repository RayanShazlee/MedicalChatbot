...
#     )
#
#     # ✅ Fix: Clamping scores at the `vectorstore` level, NOT `retriever`
#     def custom_filter(results):
#         return [(doc, min(max(score, 0), 1)) for doc, score in results]
#
#     # Overriding similarity search at the `vectorstore` level
#     original_method = vector_store.similarity_search_with_relevance_scores
#     vector_store.similarity_search_with_relevance_scores = lambda query, **kwargs: custom_filter(
#         original_method(query, **kwargs)
#     )
#
...
#     app.run(host="0.0.0.0",x port=8080, debug=os.getenv("FLASK_DEBUG", False))
#----------------------------------------------------------------------------------------------
import os
...
#
# # Initialize Google Translator
# # ✅ Language Detection Function (Fixed)
# def detect_language(text):
#     """ Detects the language of the given text """
#     try:
#         detected_lang = GoogleTranslator().detect(text)  # Returns language code (e.g., 'hi' for Hindi)
#         print(f"Detected Language: {detected_lang}")
...
# # ✅ Configuration
# class Config:
#     PINECONE_API_KEY = os.getenv("PINECONE_API_KEY") or getpass.getpass("Pinecone API Key (set in .env): ")
#     GROQ_API_KEY = os.getenv("GROQ_API_KEY") or getpass.getpass("Groq API Key (set in .env): ")
#     INDEX_NAME = "medical-chatbot"
#     EMBEDDING_MODEL = "sentence-transformers/all-MiniLM-L6-v2"
#     LLM_MODEL = "llama-3.3-70b-versatile"
#     LLM_TEMPERATURE = 0.8
...
#         final_response = GoogleTranslator(source="en", target=detected_lang_code).translate(response_text)
#         print(f"Final Translated Response: {final_response}")
#
